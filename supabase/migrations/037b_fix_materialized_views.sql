...
SELECT
  ...
  SUM(winner_contracts)::int AS total_contracts,
  COALESCE(SUM(winner_volume), 0)::bigint AS total_volume,
  ROUND(SUM(share_pct * share_pct))::int AS hhi
FROM (
  SELECT
    cpv_sector,
    winner_name,
    COUNT(*) AS winner_contracts,
    COALESCE(SUM(amount), 0) AS winner_volume,
